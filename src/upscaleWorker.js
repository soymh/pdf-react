...
addEventListener('message', async (e) => {
  ...
  if (data?.model === "anime_4x") {
    model_url = `/models/anime_4x/model.json`;
  }
  if (data?.model === "anime_4x_plus") {
    model_url = `/models/anime_4x_plus/model.json`;
  }
  if (data?.model === "general") {
    model_url = `/models/general/model.json`;
  }
  if (data?.model === "realx4plus") {
    model_url = `/models/realx4plus/model.json`;
  }
  if (data?.model === "realx4v3-fast") {
    model_url = `/models/realx4v3-fast/model.json`;
  }
